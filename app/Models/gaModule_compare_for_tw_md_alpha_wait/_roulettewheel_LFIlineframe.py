...
class GeneticAlgo:
    ...
    def crossover(self, parent1, parent2):      # parent1, parent2, child are all "tour"s
        ...
        #parent1이라는 하나의 투어 내부에서 길이를 지정하고 그 길이만큼 오른쪽벽에 닿을때까지만 프레임을 잡는다.
        #해당 루트의 시작노드,끝노드의 인덱스를key로 피트니스값을value로 하는 딕셔너리로서 저장 
        #portionstart,end는 투어의 맨앞노드, 맨뒤노드를 의미한다. 맨뒤노드의 뒤노드가 아니라!
        routefitnessdict = {}
        for portionstart in range(0, round(parent1.toursize()/2+0.1)+1):
            historynode = []
            for commondiff in range(0, parent1.toursize()//2):
